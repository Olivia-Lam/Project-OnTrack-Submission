...
INPUT_VIDEO = Path("videos/cv5/cv5_mrt.mp4")
OUTPUT_VIDEO = Path("videos/cv5/cv5_mrt_anonymised.mp4")
...
# Approximate upper portion of person bounding box
FALLBACK_HEAD_TOP = 0.00
FALLBACK_HEAD_BOTTOM = 0.38

# Prevent the fallback from being used for extremely
# small detections that are unlikely to be useful.
...
print()
print("=" * 60)
print("SMARTGS VIDEO ANONYMISATION")
print("=" * 60)
...
def blur_region(frame, x1, y1, x2, y2):
    ...
    # Make sure the blur kernel is valid
    kernel_width = min(
        BLUR_SIZE,
        region.shape[1]
    )
...
while True:
    ...
    for face_box in detected_faces:
        ...
        # Expand face detection to approximate
        # the entire head.
        head_x1 = int(
            x1 -
            face_width *
    ...
    for person_box in detected_people:
        ...
        # Check whether this person already has
        # a detected face.
        face_found = False
        ...
        # If a face was found, the head has already
        # been anonymised.
        if face_found:
            continue
        ...
        #
        # Use the upper portion of the person box
        # as a fallback head region.
        # ----------------------------------------------------
        ...
        # Slightly shrink the width so we don't
        # unnecessarily blur the shoulders.
